...
template<typename F>
__global__ void kernel_forward(const int B, const int T, const int C, const int H, const int W,
    ...
    const F inv_W = 1.0 / W, inv_H = 1.0 / H;
    // for saving smem, del Sc, Sd, So1
    __shared__ F Sa[CHANNEL_SPLIT][TOKEN_SPLIT], Sb[CHANNEL_SPLIT][TOKEN_SPLIT], So2[CHANNEL_SPLIT][TOKEN_SPLIT];
    F a = 0, b = 0, c = 0, d = 0;
    F o1 = MIN_VALUE, o2 = MIN_VALUE;
    int t_h = _t / W, t_w = _t % W;
    int te_h = (_t + _tokenLength) / W, te_w = (_t + _tokenLength) % W;
    for (int i = _t; i < (_t + _tokenLength); i++) {
        const int i_h = i / W, i_w = i % W;
        const F rd = abs(i_h - t_h) * inv_H + abs(i_w - t_w) * inv_W;
        const int ii = i * C;
        F no = max(o1, k[ii] - w * rd);
        F e1 = exp(o1 - no);
        F e3 = exp(k[ii] - w * rd - no);
        c = e1 * c + e3 * v[ii];
        d = e1 * d + e3;
        o1 = no;
        const int ni = 2 * _t + _tokenLength - 1 - i;
        const int nini = ni * C;
        const int ni_h = ni / W, ni_w = ni % W;
        const F exp_w = abs(te_h - ni_h) * inv_H + abs(te_w - ni_w) * inv_W;
    ...
    b = 0;
    o2 = MIN_VALUE;
    te_h = ((token_id - 1) * _T) / W, te_w = ((token_id - 1) * _T) % W;
    for (int i = 0; i < token_id; i++) {
        const int i_h = (i * _T) / W, i_w = (i * _T) % W;
        const F exp_w = abs(i_h - te_h) * inv_H + abs(i_w - te_w) * inv_W;
    ...
    d = 0;
    o1 = MIN_VALUE;
    te_h = ((TOKEN_SPLIT - 1) * _T) / W, te_w = ((TOKEN_SPLIT - 1) * _T) % W;
    ...
    for (int i = _t; i < (_t + _tokenLength); i++) {
        ...
        b = e2 * b + e3;
        o2 = no;
        F rd = abs(ii2 / W - (ii2 + 1) / W) * inv_H + abs(ii2 % W - (ii2 + 1) % W) * inv_W;
        no = max(o1 + w, k[ii2] - rd * w);
        e1 = exp(o1 + w - no);
        e3 = exp(k[ii2] - rd * w - no);
        c = e1 * c - e3 * v[ii2];
        d = e1 * d - e3;
        o1 = no;
    }
...
template<typename F>
__global__ void kernel_backward(const int B, const int T, const int C, const int H, const int W,
                                const F *__restrict__ const _w, const F *__restrict__ const _u,
                                const F *__restrict__ const _k, const F *__restrict__ const _v,
                                const F *__restrict__ const _gy,
                                F *__restrict__ const _gw, F *__restrict__ const _gu, F *__restrict__ const _gk,
    ...
    for (int i = 0; i < token_id; i++) {
        ...
        b = b * exp(o2 - no) + Sb[channel_id][i] * exp(So2[channel_id][i] - w * exp_w - no);
        dadw = dadw * exp(o2 - no) +
               (Sdadw[channel_id][i] - exp_w * Sa[channel_id][i]) * exp(So2[channel_id][i] - w * exp_w - no);
        dbdw = dbdw * exp(o2 - no) +
               (Sdbdw[channel_id][i] - exp_w * Sb[channel_id][i]) * exp(So2[channel_id][i] - w * exp_w - no);
        o2 = no;
    }
    __syncthreads();
    So2[channel_id][token_id] = o1;
...
void cuda_forward(int B, int T, int C, int H, int W, float *w, float *u, float *k, float *v, float *y) {
    ...
    kernel_forward<<<numBlocks, threadsPerBlock>>>(B, T, C, H, W, w, u, k, v, y);
}

void cuda_backward(int B, int T, int C, int H, int W, float *w, float *u, float *k, float *v,
                   float *gy, float *gw, float *gu, float *gk, float *gv) {
    dim3 threadsPerBlock(min(CHANNEL_SPLIT, C), TOKEN_SPLIT); // requires --maxrregcount 60 for optimal performance
    assert(B * C % threadsPerBlock.x == 0);
    dim3 numBlocks(B * C / threadsPerBlock.x);
    kernel_backward<<<numBlocks, threadsPerBlock>>>(B, T, C, H, W, w, u, k, v, gy, gw, gu, gk, gv);
}
