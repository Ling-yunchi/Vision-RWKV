...
def vrwkv6_flops(n, dim, head_size):
    addi_flops = 0
    addi_flops += n * dim * (TIME_MIX_EXTRA_DIM * 10 +
                             TIME_DECAY_EXTRA_DIM * 2 + 7 * head_size + 17)
...
def get_addi_flops_vit(model, input_shape, cfg):
    ...
    print(
        f"Additional Flops in Vit(Attn)*{len(model.backbone.layers)}: {flops_to_string(addi_flops)}")
    return addi_flops
...
def get_addi_flops_vrwkv6(model, input_shape, cfg):
    ...
    print(
        f"Additional Flops in VRWKV6*{num_layers} layers: {flops_to_string(addi_flops)}")
    return addi_flops
...
def get_addi_flops_vrwkv(model, input_shape, cfg):
    ...
    print(
        f"Additional Flops in VRWKV(Attn)*{num_layers} layers: {flops_to_string(addi_flops)}")
    return addi_flops


def get_flops(model, input_shape, cfg, ost):
    flops, params = get_model_complexity_info(
        model.backbone, input_shape, as_strings=False, ost=ost)
    model_name = type(model.backbone).__name__
    print(model_name)
    if 'VisionTransformer' in model_name:
        add_flops = get_addi_flops_vit(model.backbone, input_shape, cfg)
        flops += add_flops
    elif model_name == 'VRWKV':
        add_flops = get_addi_flops_vrwkv(model.backbone, input_shape, cfg)
        flops += add_flops
    elif model_name == 'VRWKV6':
        add_flops = get_addi_flops_vrwkv6(model.backbone, input_shape, cfg)
        flops += add_flops
